  ...
  run: () => {
    ...
    config.prePush.forEach(prePushTask => {
      ...
      if (task) {
        ...
        console.error(
          chalk.red(
            `Pre-push task ${prePushTask.name} is not found and is not executed before pushing`
          )
        );
      }
    });
    ...
    Promise.all(tasks).then(() => {
      ...
      prePush.errors.forEach(error => {
        const lintError = error.outputPath
          ? fs.readFileSync(error.outputPath, 'utf8')
          : error.stdout;
        console.error(
          chalk.yellow(
  ...
  tasks: [
    {
      ...
      run: options => {
        ...
        return pify(shell.exec)(
          `${linterBinary} ${sourceDir} --ext .js -o ${outputPath} --cache`
        ).catch((error, stdout, stderr) =>
          prePush.errorHandler(error, stdout, stderr, linter, outputPath)
        );
      },
    },
    {
      ...
      run: options => {
        ...
        return pify(shell.exec)(
          `${linterBinary} "${sourceDir}" ${excludeArgument} -o ${outputPath} -p ${tsConfig} -c ${lintingRules}`
        ).catch((error, stdout, stderr) =>
          prePush.errorHandler(error, stdout, stderr, linter, outputPath)
        );
      },
    },
    {
      ...
      run: options => {
        ...
        return pify(shell.exec)(
          `${linterBinary} "${sourceDir}" > ${outputPath} --cache`
        ).catch((error, stdout, stderr) =>
          prePush.errorHandler(error, stdout, stderr, linter, outputPath)
        );
      },
